...
// require('/fpdfFunction/html2pdf.php');
// require('/fpdfFunction/ean13.php');
// require('/fpdfFunction/i25.php');
...
// print(count($formatedData).'<br/>');
// foreach ($formatedData as $roomKey => $roomValue) {
//   print($roomKey.' '.count($roomValue).'<br/>');
//   foreach ($roomValue as $studentKey => $studentValue) {
//     print($studentKey.' '.$studentValue['studentId'].' '.$studentValue['firstname'].' '.$studentValue['lastname'].' '.$studentValue['no'].'<br/>');
//   }
// }
  ...
  foreach ($roomValue as $studentKey => $studentValue) {
    ...
    if($i < 12) {
      if ($j < 4) {
        ...
        // <img alt="testing" src="createBarcode.php?size=120&sizefactor=2.5&text='.$studentId.'"/>
        // $pdf->Image("/createBarcode.php?size=120&sizefactor=2.5&text='.$studentId.'",null,null,50,$barcodeHeight);
        // $pdf->WriteHTML('<img alt="testing" src="createBarcode.php?size=120&sizefactor=2.5&text='.$studentId.'"/> test"');
        // $pdf->Image("/createBarcode.php?size=120&sizefactor=2.5&text='.$studentId.'",null,null,50,$barcodeHeight);
        // $pdf->Image(barcode( null, $studentId, 120, null, null, null, 2.5 ),null,null,50,$barcodeHeight);
        // $pdf->Image('http://localhost:8888/createBarcode.php?size=120&sizefactor=2.5&text='.$studentId,null,null,50,$barcodeHeight,'png');
        // $pdf->Image('./img/52748.png',null,null,50,$barcodeHeight);
        
        // $pdf->Barcode($startLeft,$startTop + 1,$studentId,$barcodeHeight,null,10);
        // $pdf->UPC_A($startLeft,$startTop + 1,$studentId, $barcodeHeight);
        // $pdf->EAN13($startLeft,$startTop + 1,$studentId, $barcodeHeight);
        // $pdf->i25($startLeft,$startTop + 1,$studentId);
...
//       $pdf->Cell($width,$studentIdHeight ,$studentId,0,2,'C');
//       $pdf->SetFont('THSarabunNew','',10);
//       $pdf->Cell($width,$nameHeight ,$fisrtName.' '.$lastname,0,2,'C');
//       $pdf->Cell($width,$roomHeight,$room.' '.$no,0,2,'C');
//       $pdf->SetXY($startLeft,$startTop);
//       $startLeft = $startLeft + $width;
//       $j = $j + 1;
